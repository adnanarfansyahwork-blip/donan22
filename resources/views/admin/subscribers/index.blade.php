...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <form action="{{ route('admin.subscribers.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.subscribers.broadcast') }}" 
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form id="bulk-form" action="{{ route('admin.subscribers.bulk-delete') }}" method="POST">
            @csrf
            @method('DELETE')
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($subscribers as $subscriber)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <input type="checkbox" name="ids[]" value="{{ $subscriber->id }}" class="subscriber-checkbox rounded border-gray-300">
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex justify-end gap-2">
                                        <form action="{{ route('admin.subscribers.toggle-status', $subscriber) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-blue-600 hover:text-blue-900" title="{{ $subscriber->status === 'active' ? 'Deactivate' : 'Activate' }}">
                                                @if($subscriber->status === 'active')
