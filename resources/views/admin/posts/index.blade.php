    ...
    <form method="GET" action="{{ route('admin.posts.index') }}" class="flex flex-wrap items-end gap-4">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search', 'status', 'type', 'category']))
                <a href="{{ route('admin.posts.index') }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($posts as $post)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $post->status_badge_class }}">
                                {{ $post->status_label }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3 text-sm text-gray-500">
                                ...
                                <span title="Downloads">
                                    <i class="bi bi-download"></i> {{ number_format($post->downloads_count) }}
                                </span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
...
<div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <div class="bg-white rounded-lg p-4 shadow-sm">
        <div class="text-2xl font-bold text-green-600">{{ \App\Models\Post::published()->count() }}</div>
        <div class="text-sm text-gray-500">Published</div>
    </div>
    <div class="bg-white rounded-lg p-4 shadow-sm">
        <div class="text-2xl font-bold text-yellow-600">{{ \App\Models\Post::draft()->count() }}</div>
        <div class="text-sm text-gray-500">Drafts</div>
    </div>
    <div class="bg-white rounded-lg p-4 shadow-sm">
        <div class="text-2xl font-bold text-blue-600">{{ \App\Models\Post::scheduled()->count() }}</div>
